  ...
  return (
    <ProductContainer>
      ...
      <BaseInfo>
        ...
        <div>Price: ${(props.location.productData || altData).price}</div>
        <div>Rating: {(props.location.productData || altData).rating} / 5</div>
      </BaseInfo>
      <br />
        ...
        <CartButton
          onClick={() => {
            store.dispatch({
              type: "ADD_TO_CART",
              data: {
                productImg: (props.location.productData || altData).img,
                productName: (props.location.productData || altData).title,
                productPrice: (props.location.productData || altData).price,
                quantity: productCount,
              },
            });
            history.replace("/eCommerceStore/cartPage");
          }}
        >
          Add to Cart
        </CartButton>
